content = '''\"""Bithumb REST API adapter for experiment_v1.
...
class BithumbSession:
    ...
    def place_order(self, category=None, symbol: str = "", side: str = "",
        ...
        if self._is_krw_market(market):
            rate = self._krw_usdt_rate()
            # price arrives in USDT from runner.py -- convert back to integer KRW
            krw_price = str(round(float(price) * rate)) if price is not None else None
        else:
...
with open(r'C:\Users\nasan\crypto\strategies\experiment_v1\bithumb_exchange.py', 'w', encoding='utf-8') as f:
    f.write(content)
print('written ok')
